...
void unicodeToUTF8(const wstring& src, string& result)
{
    ...
    try {
        ...
        OutPutStr = new char[InPutStrLen * 8 + 8];
    }
    catch (std::bad_alloc) {
...
void utf8ToUnicode(const string& src, wstring& result)
{
    ...
    wchar_t* Unic = NULL;
    try {
        Input = new unsigned char[src.length() + 1];
        Unic = new wchar_t[src.length() + 1];
    }
    catch (std::bad_alloc) {
        std::cerr << "bad_alloc" << endl;
        return;
    ...
    while (*pInput != 0)
    {
        ...
        unsigned char* pOutput = (unsigned char*)&uchar;
