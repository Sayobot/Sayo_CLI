	...
	try
	{
		while (cur_dev) {
...
#endif
			{
				...
				if (this->handle)
				{
					printf("  output_report_length: %d\n", this->handle->output_report_length);
					printf("  input_report_length: %d\n", this->handle->input_report_length);
					...
					hid_write(this->handle, this->EpOut.udata, this->handle->output_report_length);
					if (hid_read_timeout(this->handle, this->EpIn.udata, this->handle->input_report_length, 2000) > 0)
					{
...
int O2Protocol::Check()
{
	...
	hid_write(this->handle, this->EpOut.udata, this->handle->output_report_length);
	if (hid_read_timeout(this->handle, this->EpIn.udata, this->handle->input_report_length, HID_TIMEOUT) > 0)
	{
		return 0;
	}
	else
				...
				for (unsigned i = 0; i < format["data"].size(); i++)
				{
					...
					if (format["data"][i]["type"].asString() == "cmd")
					{
						...
						for (int c = 0; c < this->EpOut.format.data_len + 3; c++)
							OUT_CHECKSUM += this->EpOut.data[c];
						hid_write(this->handle, this->EpOut.udata, this->handle->output_report_length);
						if (hid_read_timeout(this->handle, this->EpIn.udata, this->handle->input_report_length, 1000) > 0)
						{
...
string O2Protocol::Script(const Json::Value& data)
{
		...
		else
		{
			...
			for (i = 0; i < addr;)
			{
				...
				case S_GA_XYZ_VAL:
					...
					break;
				case S_TB_XY:
					this->script[num].value[0] = *(USHORT*)&buf[i]; i += 2;
					this->script[num].value[1] = *(USHORT*)&buf[i]; i += 2;
					break;
	...
	else if (data["method"].asString() == "write")
	{
		...
		for (unsigned step = 0; this->script[step].code != 0; step++)
		{
			...
			case S_TB_XY:
				*(USHORT*)&buf[i] = this->script[step].value[0]; i += 2;
				*(USHORT*)&buf[i] = this->script[step].value[1]; i += 2;
				break;
		...
		for (i = 0; i < addr;)
		{
			...
			hid_write(this->handle, this->EpOut.udata, this->handle->output_report_length);
			Sleep(5);
			if ((err = hid_read_timeout(this->handle, this->EpIn.udata, this->handle->input_report_length, HID_TIMEOUT)) > 0)
			{
	...
	if (data["method"].asString() == "read")
	{
		...
		while (1)
		{
			...
			for (int i = 0; i < this->EpOut.format.data_len + 3; i++)
				OUT_CHECKSUM += this->EpOut.data[i];
			hid_write(this->handle, this->EpOut.udata, this->handle->output_report_length);
			if (hid_read_timeout(this->handle, this->EpIn.udata, this->handle->input_report_length, HID_TIMEOUT*2) > 0)
			{
				if (this->EpIn.format.cmd == 0)
				{
